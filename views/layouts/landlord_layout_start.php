...
<head>
    ...
    <title><?= $siteName ?> | Landlord Portal</title>
    <?php if($faviconUrl): ?>
...
<body class="bg-gray-100 font-sans text-gray-900 overflow-x-hidden">
    ...
    <aside id="sidebar" class="fixed top-0 left-0 z-50 h-screen w-64 bg-slate-900 text-slate-300 sidebar-transition transform -translate-x-full lg:translate-x-0 overflow-y-auto">
        ...
        <nav class="p-4 space-y-2">
            ...
            <a href="<?= APP_URL ?>/landlord/dashboard" class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'landlord/dashboard') !== false) ? 'bg-blue-600 text-white shadow-md' : '' ?>">
