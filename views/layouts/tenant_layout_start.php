...
<head>
    ...
    <title>Tenant Dashboard | ZZY Rental</title>
    ...
    <aside id="sidebar" class="fixed top-0 left-0 z-50 h-screen w-64 bg-slate-900 text-slate-300 sidebar-transition transform -translate-x-full lg:translate-x-0 overflow-y-auto">
        <div class="flex items-center justify-between p-6 border-b border-slate-800">
            <div class="flex items-center gap-3">
                ...
                <span class="text-xl font-bold text-white tracking-tight">Tenant Hub</span>
            </div>
        ...
        <nav class="p-4 space-y-2">
            ...
            <a href="<?= APP_URL ?>/tenant/dashboard" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'tenant/dashboard') !== false) ? 'bg-blue-600 text-white shadow-md' : '' ?>">
                <i class="fas fa-chart-pie w-5"></i> <span>Dashboard</span>
            </a>

            <a href="<?= APP_URL ?>/tenant/verify" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'tenant/verify') !== false) ? 'bg-blue-600 text-white shadow-md' : '' ?>">
    ...
    <div class="lg:ml-64 min-h-screen flex flex-col">
        ...
        <header class="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6 sticky top-0 z-30">
            <div class="flex items-center gap-4">
                ...
                <h2 class="text-lg font-semibold text-gray-800 hidden sm:block">Tenant Portal</h2>
            </div>
