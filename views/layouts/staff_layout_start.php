...
<body class="bg-gray-50 font-sans text-gray-900 overflow-x-hidden">
    ...
    <aside id="sidebar" class="fixed top-0 left-0 z-50 h-screen w-64 bg-slate-900 text-slate-300 sidebar-transition transform -translate-x-full lg:translate-x-0 overflow-y-auto">
        <div class="flex items-center justify-between p-6 border-b border-slate-800">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-xl bg-violet-600 flex items-center justify-center text-white font-bold shadow-lg overflow-hidden border border-white/10">
                    <?php if ($userAvatar): ?>
                ...
                <span class="text-xl font-bold text-white tracking-tight sidebar-header-text"><?= htmlspecialchars($_SESSION['username'] ?? 'Staff') ?></span>
            </div>
        ...
        <nav class="p-4 space-y-1">
            ...
            <a href="<?= APP_URL ?>/staff/dashboard" class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'staff/dashboard') !== false) ? 'active-link' : '' ?>">
            ...
            <a href="<?= APP_URL ?>/staff/pending" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'staff/pending') !== false) ? 'active-link' : '' ?>">
                <i class="fas fa-clock w-5 text-yellow-400"></i> <span class="sidebar-text">Pending Properties</span>
                <?php
                try {
                    $db = Database::getInstance()->getConnection();
            ...
            <a href="<?= APP_URL ?>/admin/reviews" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'admin/reviews') !== false) ? 'active-link' : '' ?>">
            ...
            <a href="<?= APP_URL ?>/messages" class="flex items-center justify-between px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'messages') !== false) ? 'active-link' : '' ?>">
                <div class="flex items-center gap-3">
                    <i class="fas fa-envelope w-5 text-violet-400"></i> <span class="sidebar-text">Chat</span>
                </div>
                <?php if ($unreadMsgs > 0): ?>
                    <span class="bg-violet-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full"><?= $unreadMsgs ?></span>
                <?php endif; ?>
            </a>

            <a href="<?= APP_URL ?>/staff/cms" class="sidebar-item flex items-center gap-3 px-4 py-3 rounded-xl transition-all hover:bg-slate-800 hover:text-white <?= (strpos($_SERVER['REQUEST_URI'], 'staff/cms') !== false) ? 'bg-blue-600 text-white shadow-md' : '' ?>">
    ...
    <div id="mainContent" class="lg:ml-64 min-h-screen flex flex-col sidebar-transition">
        ...
        <header class="bg-white border-b border-gray-200 h-16 flex items-center justify-between px-6 sticky top-0 z-30 shadow-sm">
            ...
            <div class="flex items-center gap-3">
                <a href="<?= APP_URL ?>/messages" class="relative text-gray-400 hover:text-violet-600 transition p-2 mr-2">
                ...
                <a href="<?= APP_URL ?>/notifications" class="relative text-gray-400 hover:text-gray-600 transition p-2 mr-2">
                    ...
                    <button class="flex items-center gap-3 pl-4 border-l border-gray-200 focus:outline-none">
                        <div class="text-right hidden sm:block">
                            <p class="text-sm font-bold text-gray-800 leading-none"><?= $_SESSION['username'] ?? 'Staff' ?></p>
                            <p class="text-[10px] text-gray-500 uppercase font-semibold">Service Staff</p>
                        </div>
